		   ...
		   <table class="main">
				...
				<c:if test="${result == 1}">
					...
					<tr>
						...
						<td>${registerEm.name}</td>
					</tr>
					<tr>
						<td style="width: 50%;">레벨</td>
						<td>
							<c:if test="${registerEm.level == 'A'}">운영자</c:if>
							<c:if test="${registerEm.level == 'B'}">일반회원</c:if>
						</td>
					</tr>
					<tr>
						<td style="width: 50%;">성별</td>
						<td>
							<c:if test="${registerEm.gender == 1}">남자</c:if>
							<c:if test="${registerEm.gender == 2}">여자</c:if>
						</td>
					</tr>
					<tr>
						<td style="width: 50%;">전화번호</td>
						<td>${registerEm.phone}</td>
					</tr>
				</c:if>
					<tr>
						<td colspan="2">
